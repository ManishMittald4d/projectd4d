...
import { setStudentFilterData } from 'store/readability/readabilitySlice';
...
const CustomerTableFilter = () => {
	...
	const { status } = useSelector((state) => state?.readability?.studentFilterData)

	const onStatusFilterChange = (selected) => {
		dispatch(setStudentFilterData({status: selected.value}))
	}
