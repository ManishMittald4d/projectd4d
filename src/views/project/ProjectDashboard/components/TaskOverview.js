...
const PriorityTag = (props) => {
	...
	switch (priority) {
		case 0:
			return <div onClick={(e) => handleClick(e, 'graph1')}><Tag className="overflowContentHandle cursor-pointer py-4 w-full text-red-600 bg-red-100 dark:text-red-100 dark:bg-red-500/20 rounded border-0" ><Button shape="circle" variant="plain"  size="xs" icon={<MdHistory />} />Duration</Tag></div>	
		case 1:
			return <div onClick={(e) => handleClick(e, 'graph2')}><Tag className="overflowContentHandle cursor-pointer py-4 w-full text-amber-600 bg-amber-100 dark:text-amber-100 dark:bg-amber-500/20 rounded border-0" onClick={(e) => handleClick(e, 'graph2')} ><Button shape="circle" variant="plain"  size="xs" icon={<HiCheck />} />Accuracy</Tag></div>
		case 2:
			return <div onClick={(e) => handleClick(e, 'graph3')}><Tag className="overflowContentHandle cursor-pointer py-4 w-full bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-100 rounded border-0" ><Button shape="circle" variant="plain"   size="xs" icon={<HiQuestionMarkCircle />} />Comprehension</Tag></div>
		case 3:
				return <div onClick={(e) => handleClick(e, 'graph4')}><Tag className="overflowContentHandle cursor-pointer py-4 w-full bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-100 rounded border-0"><Button shape="circle" variant="plain"  size="xs" icon={<MdMenuBook />} />Books Read</Tag></div>
		case 4:
				return <div onClick={(e) => handleClick(e, 'graph5')}><Tag className="overflowContentHandle cursor-pointer py-4 w-full bg-blue-100 text-blue-600 dark:bg-blue-500/20 dark:text-blue-100 rounded border-0" ><Button shape="circle" variant="plain"  size="xs" icon={<MdMenuBook />} />Gade Wise Books Read</Tag></div>				
		default:
			return null
	}
...
const TaskOverview = ({data = {}, className}) => {
	...
	const [startDate, setStartDate] = useState(dayjs().subtract(7, 'days').toDate()); //useSelector((state) => state?.salesDashboard?.state?.startDate);
	...
	return (
		<Card className={className}>
			...
			<Tooltip title={'Filter'} wrapperClass="mr-1"><Button className="bg-[#F3F6F9] mr-2" shape="normal" variant="plain" size="sm" icon={<MdFilterAlt />} />
			</Tooltip>
			</div>
			</div>
				</div>
				...
				/>
						...
						options={{
							...
							xaxis: {
								...
								categories: ['01/01/2023 GMT', '01/02/2023 GMT', '01/03/2023 GMT', '01/04/2023 GMT',
									'01/05/2023 GMT', '01/06/2023 GMT','01/07/2023 GMT','01/10/2023 GMT','01/13/2023 GMT'
								],
							},
					...
					<div className=''>
						<div className="grid grid-cols-2 gap-2 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1">
								...
								<PriorityTag onClick={(e, name) =>handleChangeTag(e, name) } className="tab-margin" priority={0} />
								</div>

								<div className='tablet-margin'><PriorityTag priority={1} onClick={(e, name) =>handleChangeTag(e, name) }/></div>
								<div className='tablet-margin'><PriorityTag priority={2} onClick={(e, name) =>handleChangeTag(e, name) }/></div>
								<div className='tablet-margin'><PriorityTag priority={3} onClick={(e, name) =>handleChangeTag(e, name) }/></div>
								<div className='tablet-margin'><PriorityTag priority={4} onClick={(e, name) =>handleChangeTag(e, name) }/></div>
								<div className='tablet-margin'><PriorityTag priority={5} onClick={(e, name) =>handleChangeTag(e, name) }/></div>
						</div>
					</div>
				</div>
			</div>
