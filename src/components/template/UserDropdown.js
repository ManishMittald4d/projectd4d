...
import { HiOutlineUser, HiOutlineCog, HiOutlineLogout,HiOutlineLockClosed} from 'react-icons/hi'
...
const dropdownItemList = [
	...
	// { label: 'Change Password', path: '/app/account/settings/Password', icon: <HiOutlineLockClosed /> },
]

export const UserDropdown = ({ className }) => {

	const { avatar, userName, authority, email,DisplayName } = useSelector((state) => state.auth.user)
	...
	const UserAvatar = (
		<div className={classNames(className, 'flex items-center gap-2')}>
			...
			<div className="hidden md:block">
				<div className="text-xs capitalize">{DisplayName || 'guest'}</div>
