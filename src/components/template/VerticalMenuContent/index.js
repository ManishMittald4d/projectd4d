...
const VerticalMenuContent = (props) => {
  ...
  const getNavItem = (nav) => {
    //console.log(NAV_ITEM_TYPE_TITLE,'titledat');
    //console.log(NAV_ITEM_TYPE_COLLAPSE,'colsdata');

    //console.log(nav.subMenu.length,'length');
    //console.log(nav.type, nav?.path, nav.type, 'submenulength');

    if (nav.subMenu.length === 0 && nav.type === NAV_ITEM_TYPE_ITEM) {
      //console.log(nav.subMenu.length,'lengthinner');
      return (
